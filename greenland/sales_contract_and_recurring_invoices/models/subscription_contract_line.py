...
class SubscriptionContractLines(models.Model):
    ...
    product_uom_qty = fields.Float(string="QTY(Min)",
    ...
    product_uom_id = fields.Many2one('uom.uom', string='Unit of Measure',
                                     ...
                                     help='Unit of measure of product',readonly=False)
    ...
    def _compute_description(self):
        """ Compute product description, slice up to the first line break, and remove data before the first ']' """
        for option in self:
            if not option.product_id:
                continue
            # Get the description using the context language
            product_lang = option.product_id.with_context(
                lang=self.subscription_contract_id.partner_id.lang)

            # Fetch the description
            description = product_lang.get_product_multiline_description_sale()
            ...
            if bracket_index != -1:
                description = description[bracket_index + 1:].strip()
